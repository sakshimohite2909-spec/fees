      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-15 sm:h-16 gap-2">
          ...
          <div className="flex items-center gap-2.5 sm:gap-3.5 shrink-0 min-w-0">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-1.5 sm:gap-2">
                ...
                <span className={`text-[9px] sm:text-[10px] font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-full border flex items-center gap-1.5 shadow-2xs ${
                  activeRole === 'admin' 
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            {currentUser && (
              <div className="flex items-center gap-2 sm:gap-2.5 bg-slate-50/90 hover:bg-white px-2.5 sm:px-3 py-1.5 rounded-2xl border border-slate-200/90 shadow-2xs transition-all">
                <div className="text-right max-w-[100px] sm:max-w-[150px] truncate">
                  <p className="text-xs sm:text-sm font-bold text-slate-900 leading-none truncate">
                    {currentUser.fullName || 'Student'}
                  </p>
                  <p className="text-[10px] text-slate-500 font-medium font-mono leading-tight mt-0.5 truncate">
                    +91 {currentUser.mobile || 'Active'}
                  </p>
                </div>
                <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-xl bg-gradient-to-br from-indigo-600 to-indigo-700 flex items-center justify-center font-black text-white text-xs sm:text-sm shadow-xs shrink-0">
                  {(currentUser.fullName?.[0] || 'S').toUpperCase()}
                </div>
                <button
                  onClick={onLogout}
                  title="Logout Account"
                  className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-white hover:bg-rose-50 text-slate-700 hover:text-rose-600 font-semibold text-xs border border-slate-200 hover:border-rose-200 transition-all cursor-pointer shrink-0 shadow-2xs group"
                >
